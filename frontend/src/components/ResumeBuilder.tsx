  ...
  const exportResume = async (format: "pdf" | "docx") => {
    ...
    try {
      const response = await fetch("/api/resumes/export", {
        ...
        body: JSON.stringify({ format, template_id: activeTemplate })
      });
      ...
      if (data.success) {
         if (format === "docx") {
            ...
            // Show print dialog or download blob
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6">
             ...
             <div className="space-y-3">
                <button
                  ...
                  className="w-full bg-slate-800 hover:bg-slate-700 disabled:opacity-50 text-white font-bold py-3 rounded-2xl flex items-center justify-center gap-2 transition-all text-xs"
                >
