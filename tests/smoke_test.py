...
async def test_resume_parser():
    ...
    result = await parse_resume(
        text="Alex Developer, senior engineer with Python, Docker, and React. Reach me at alex@example.com"
    )
...
async def test_generator():
    ...
    candidate = {
        "full_name": "Alex",
        "key_skills": ["Python"],
        "experience_highlights": ["Built a cloud platform"],
    }
...
async def test_enricher():
    ...
    logger.info(
        f"Result Source: {result[0].get('source') if isinstance(result, list) and len(result) > 0 else 'N/A'}"
    )
...
async def run_tests():
    ...
    try:
        ...
        logger.error(f"Resume Parser test failed: {e}")
