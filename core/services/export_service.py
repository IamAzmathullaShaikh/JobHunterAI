...
class ExportService:
    ...
    async def fetch_jobs_dataframe(
        ...
        stmt = select(JobListing).options(
            selectinload(JobListing.ai_analysis), selectinload(JobListing.application)
        )
        ...
        if status_filter and status_filter != "All":
            ...
            if target_enum:
                stmt = stmt.join(JobListing.application).where(
                    JobApplication.status == target_enum
                )
        ...
        for j in jobs:
            ...
            fit_summary = (
                j.ai_analysis.fit_summary if j.ai_analysis else "Pending Analysis"
            )
            ...
            rows.append(
                {
                    ...
                    "Captured Date": (
                        j.date_scraped.strftime("%Y-%m-%d %H:%M")
                        if j.date_scraped
                        else ""
                    ),
        ...
        if not df.empty:
            ...
            df = df.sort_values(
                by=["Match Score (%)", "Captured Date"], ascending=[False, False]
    ...
    async def generate_styled_excel(self, status_filter: Optional[str] = None) -> bytes:
        ...
        with pd.ExcelWriter(output, engine="openpyxl") as writer:
            ...
            link_font = Font(
                name=font_family, size=10, color="2563EB", underline="single"
            )
        ...
        output.seek(0)
        logger.info(
            "Generated polished Excel tracking workbook bytes matching DeclarativeBase schema."
        )
