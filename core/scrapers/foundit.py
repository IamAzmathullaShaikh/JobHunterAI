        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                cards = (
                    ...
                    or soup.find_all("div", class_="card-template")
                    or soup.find_all("div", class_="srpResultCard")
                )
                ...
                for card in cards[:limit]:
                    try:
                        title_tag = (
                            card.find("div", class_="card-headline")
                        ...
                        raw_id = card.get(
                            "id", f"foundit-{random.randint(100000, 999999)}"
                        )
                        comp_tag = card.find("div", class_="company-name") or card.find(
                            "span", class_="company-name"
                        )
                        company_name = (
                        ...
                        loc_tag = card.find("div", class_="location") or card.find(
                            "span", class_="loc"
                        )
                        loc_text = (
                        ...
                        desc_tag = card.find(
                            "div", class_="card-description"
                        ) or card.find("p", class_="job-desc")
                        desc_text = (
                            desc_tag.get_text(strip=True)
                            if desc_tag
                            else f"{title} at {company_name}"
                        )
