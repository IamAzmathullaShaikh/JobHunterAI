...
class IndeedScraper(BaseScraper):
    ...
    async def scrape(
        ...
        domain = (
            "in.indeed.com" if "india" in target_location.lower() else "www.indeed.com"
        )
        url = (
            f"https://{domain}/jobs?q={quote(search_query)}&l={quote(target_location)}"
        )
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                log.info(
                    f"Discovered {len(cards)} raw job listing cards in view stream."
                )

                for card in cards[:limit]:
                    try:
                        title_anchor = (
                            card.find("a", class_="jxf413")
                            or card.find("a", id=lambda x: x and x.startswith("job_"))
                        ...
                        jk_id = title_anchor.get("data-jk") or card.find_parent(
                        ...
                        snippet_tag = card.find(
                            "div", class_="jobMetaDataGroup"
                        ) or card.find("div", class_="underShelfFooter")
                        snippet_text = (
                            snippet_tag.get_text(separator="\n", strip=True)
                            if snippet_tag
                            else f"Role target for {title} at {company_name}."
                        )
                        ...
                        log.warning(
                            f"Skipping corrupt structural card: {str(card_err)}"
                        )

            except Exception as e:
                log.error(
                    f"Indeed stealth scraper encountered an execution error: {str(e)}"
                )
            finally:
                await context.close()
                await browser.close()
