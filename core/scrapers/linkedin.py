    ...
    async def scrape(
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                log.info(
                    f"Discovered {len(cards)} raw job cards. Fetching detailed descriptions..."
                )
                ...
                for card in cards[:limit]:
                    try:
                        ...
                        raw_id = card.get(
                            "data-entity-urn",
                            f"li-raw-{random.randint(100000, 999999)}",
                        ).split(":")[-1]
                        ...
                        try:
                            ...
                            if response.status == 200:
                                ...
                                ) or detail_soup.find("section", class_="description")
                                if desc_div:
                                    full_description = desc_div.get_text(
                                        separator="\n", strip=True
                                    )
                        except Exception as desc_err:
                            log.debug(
                                f"Could not extract extended detail for {raw_id}: {desc_err}"
                            )
