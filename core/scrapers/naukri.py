        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                if res.status == 200:
                    ...
                    if job_details:
                        log.info(
                            f"Successfully harvested {len(job_details)} listings via Naukri REST API!"
                        )
                        for item in job_details[:limit]:
                            raw_id = str(
                                item.get("jobId", random.randint(100000, 999999))
                            )
                            ...
                            if jd_url and not jd_url.startswith("http"):
                                ...
                                full_url = (
                                    jd_url
                                    or f"https://www.naukri.com/job-listings-{raw_id}"
                                )

                            desc = (
                                item.get("jobDescription")
                                or item.get("snippets")
                                or f"{title} at {company_name}"
                            )
                ...
                log.debug(
                    f"Direct API call bypassed: {api_err}. Proceeding to Tier 2 Network Listener."
                )
            ...
            try:
                ...
                if intercepted_jobs:
                    log.info(
                        f"Captured {len(intercepted_jobs)} listings via XHR response listener!"
                    )
                    for item in intercepted_jobs[:limit]:
                        ...
                        full_url = (
                            f"https://www.naukri.com{jd_url}"
                            if jd_url.startswith("/")
                            else jd_url or page_url
                        )

                        desc = (
                            item.get("jobDescription") or f"{title} at {company_name}"
                        )
                        if isinstance(desc, list):
                            desc = " ".join(desc)
                ...
                for card in cards[:limit]:
                    try:
                        ...
                        raw_id = card.get("data-job-id") or card.get(
                            "id", f"nk-{random.randint(100000, 999999)}"
                        )

                        comp_tag = card.find("a", class_="comp-name") or card.find(
                            "a", class_="subTitle"
                        )
                        company_name = (
                            comp_tag.get_text(strip=True)
                            if comp_tag
                            else "Confidential / Unlisted"
                        )

                        loc_tag = (
                            card.find("span", class_="loc-wrap")
                            or card.find("li", class_="location")
                            or card.find("span", class_="locWrf")
                        )
                        loc_text = (
                            loc_tag.get_text(strip=True) if loc_tag else target_location
                        )

                        desc_tag = card.find("span", class_="job-desc") or card.find(
                            "div", class_="ellipsis"
                        )
                        desc_text = (
                            desc_tag.get_text(strip=True)
                            if desc_tag
                            else f"Naukri listed role: {title}."
                        )
