...
async def cloud_generate_cover(
    ...
    response = await client.chat_completion(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are a professional technical writer creating personalized cover letters.",
            },
...
cloud_generate_cover.required_envs = [
    "GROQ_API_KEY",
    "OPENAI_API_KEY",
    "GEMINI_API_KEY",
]
...
# --- Local Fallback ---
async def local_generate_cover(
...
{{ candidate_name }}
    ...
    data = {
        ...
        "skills": ", ".join(candidate.get("key_skills", ["relevant technologies"]))[
            :100
        ],
        "achievement": candidate.get(
            "experience_highlights", ["delivering high-quality solutions"]
        )[0],
...
local_generate_cover.safe_placeholder = {
    "source": "error",
