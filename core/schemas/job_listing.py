...
class JobListingBase(BaseModel):
    ...
    work_place_type: Optional[str] = Field(
        "Onsite", description="Onsite, Hybrid, or Remote"
    )
    job_type: str = Field(
        "Full-Time", description="Full-Time, Internship, or Apprenticeship"
    )
    ...
    description_raw: str = Field(
        ..., description="Unprocessed raw job description text"
    )
    description_clean: Optional[str] = Field(
        None, description="Sanitized description text"
    )
...
class JobListingCreate(JobListingBase):
    job_id_raw: str = Field(
        ..., description="Unique platform ID string from the target job board"
    )
