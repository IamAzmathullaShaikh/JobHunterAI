...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "job_type", sa.String(length=50), server_default="Full-Time", nullable=False
        ),
    ...
    op.create_index(
        op.f("ix_job_listings_company_name"),
        "job_listings",
        ["company_name"],
        unique=False,
    )
    op.create_index(
        op.f("ix_job_listings_job_id_raw"), "job_listings", ["job_id_raw"], unique=True
    )
    op.create_index(
        op.f("ix_job_listings_title"), "job_listings", ["title"], unique=False
    )
    op.create_index(
        op.f("ix_job_listings_user_id"), "job_listings", ["user_id"], unique=False
    )
