...
from core.database.models import AIAnalysis
from core.schemas.ai import AIAnalysisCreate, AIAnalysisDTO
...
class AIAnalysisRepository:
    ...
    async def save_analysis(
        self, job_id: int, analysis_data: AIAnalysisCreate
    ) -> AIAnalysisDTO:
        """Saves structured OpenAI output models mapped directly to an explicit target job ID."""
        logger.info(
            f"Saving match evaluation vectors for internal job mapping index: {job_id}"
        )
