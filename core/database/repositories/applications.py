...
from core.schemas.ai import (ApplicationStatusDTO, JobApplicationCreate,
                             JobApplicationDTO)
...
class ApplicationRepository:
    ...
    async def initialize_application(
        self, app_data: JobApplicationCreate
    ) -> JobApplicationDTO:
    ...
    async def update_status(
        self, app_id: int, new_status: ApplicationStatusDTO, notes: Optional[str] = None
    ) -> JobApplicationDTO:
        ...
        if not db_app:
            raise ValueError(
                f"Target system pipeline tracking tracking sequence ID {app_id} does not exist."
            )
